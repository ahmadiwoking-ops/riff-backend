...
async function runMatching(userId) {
  ...
  var candidates = await prisma.user.findMany({ where: { id: { not: userId, notIn: blockedIds } }, select: { id: true, alias: true, connectionType: true, matchVector: true, idVerified: true, trustScore: true, plan: true, avatarEmoji: true, avatarColour: true, displayPhoto: true, area: true, shareLocation: true } });
  ...
  for (var i = 0; i < filtered.length; i++) {
    ...
    if (score.overall >= 40) scored.push({ userId: o.id, alias: o.alias, score: score.overall, breakdown: score.breakdown, idVerified: o.idVerified === true, trustScore: o.trustScore || 'green', avatarEmoji: o.avatarEmoji || null, avatarColour: o.avatarColour || null, area: o.shareLocation ? (o.area || null) : null });
  }
...
// Exposed so /api/connections can show the same breakdown as a match card.
